<template>
  <div>
    <div class="d-flex m-3 justify-content-between ">
      ...
      <h1 v-else>{{ head1 }}</h1>
      <button @click="createNew" v-if="editRequest?.editreq" class="btn btn-sm btn-success fw-bold" >Create New </button>
    </div>
    <div class="border border-2 rounded">
      <form class="m-2 p-3" @submit.prevent="addNewEmployee">
        <div class="row g-3">
          <div>
            <div v-if="editRequest?.editreq" class="col">
              <input
                type="text"
                class="form-control"
